...
{ 
	...
	strx3=strstr((const char*)USART2_RX_BUF,(const char*)"blefigg");		//判断蓝牙是否自动连接
  ...
  if(USART1_RX_STA&0x8000)
	{
		...
		strx=strstr((const char*)USART1_RX_BUF,(const char*)"->ble");			//判断总线数据是否为传输给ble的数据
  ...
  if(USART2_RX_STA&0x8000)
	{
		...
		strx=strstr((const char*)USART2_RX_BUF,(const char*)"->ble");					//判断总线数据是否为传输给ble的数据
		strx2=strstr((const char*)USART2_RX_BUF,(const char*)"blefig");				//判断蓝牙是否自动连接
		
		if(strx2)		
			printf("blefigg\r\n");																							//蓝牙已连接
		
		if(strx)																															//如果数据为传输给ble的数据则取出数据显示
		{	
			switch(len)
			{
				case 9: 
						number2 = (USART2_RX_BUF[8]-0x30);
				break;
				case 10: 
						number2 = ((USART2_RX_BUF[9]-0x30)+(USART2_RX_BUF[8]-0x30)*10);
				break;
				case 11: 
						number2 = ((USART2_RX_BUF[10]-0x30) + (USART2_RX_BUF[9]-0x30)*10+(USART2_RX_BUF[8]-0x30)*100);
				break;
				case 12: 
						number2 = ((USART2_RX_BUF[11]-0x30)+ (USART2_RX_BUF[10]-0x30)*10 + (USART2_RX_BUF[9]-0x30)*100+(USART2_RX_BUF[8]-0x30)*1000);
				break;
				default : break;
			}
				delay_ms(300);						//延时300ms使数据稳定
