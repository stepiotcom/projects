...
uint32_t USART1_IRQHand(void)
{
	char *strx=0;									//接收对比结果
	static uint32_t number = 0;  	//存放接收到的数据
	static uint32_t num = 0;  		//存放传参数据
	uint8_t len;  								//存放接收到的数据长度
  ...
  if(USART1_RX_STA&0x8000)
	{
		...
		strx=strstr((const char*)USART1_RX_BUF,(const char*)"->led");			//判断总线数据是否为传输给led的数据
		
		if(strx)										//如果数据为传输给led的数据则取出数据显示
		{	
			switch(len)
			{
				case 9: 
			...
			if(number <= 5)  num = 1;	
			if(number > 5 && number <= 200)  num = 2;
			if(number > 200 && number <= 800)  num = 3;	
			if(number > 800 && number <= 2000)  num = 4;
		}
