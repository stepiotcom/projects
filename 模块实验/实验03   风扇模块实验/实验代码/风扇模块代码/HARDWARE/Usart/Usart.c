  ...
  if(USART1_RX_STA&0x8000)
	{
		...
		strx=strstr((const char*)USART1_RX_BUF,(const char*)"->fan");			//判断总线数据是否为传输给fan的数据
		
		if(strx)									//如果数据为传输给fan的数据则取出数据显示
		{
			switch(len)  							//接收到的数据转换后赋值给number1
			{
				case 9:
						delay_ms(300);  		//延时消抖				
						number1 = USART1_RX_BUF[8]-0x30;		//接收数据
						switch(number1)  		//判断数据
						{
							case 1:
									PWM_SetTIM4Compare2(10);	//驱动风扇
									delay_ms(200);							
