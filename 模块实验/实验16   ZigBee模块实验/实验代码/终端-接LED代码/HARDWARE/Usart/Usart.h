...
uint8_t* gsm_check_cmd(uint8_t *str);						//gsm_send_cmd发送命令后,检测接收到的应答
void gsm_send_cmd(uint8_t *cmd,uint8_t *ack,uint16_t waittime);	//向gsm_send_cmd发送命令

//如果想串口中断接收，请不要注释以下宏定义
void UART2_Init(int bound);
extern uint8_t RecvSta;
extern uint8_t GetMsg;
#endif
