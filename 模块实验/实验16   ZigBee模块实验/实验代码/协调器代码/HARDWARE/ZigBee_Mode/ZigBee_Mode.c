...
enum ZBM_ErrorCode ZBM_Send_P2P(uint16_t shortaddr,uint8_t *pdata,uint8_t len)
{
    uint8_t index=0,i=0;
    enum ZBM_ErrorCode error;
    ...
    ZBM_CheckRsp(1000,&USART2_RX_STA,1);
    if((USART2_RX_BUF[0] == 0xF7)&&(USART2_RX_BUF[1] == 0xFF))
    {
        error = ZBM_FAILED;
    }
    else
