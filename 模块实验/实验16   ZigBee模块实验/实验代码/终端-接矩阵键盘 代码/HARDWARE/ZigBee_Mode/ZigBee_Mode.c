...
void ZBM_Send_P2P(uint16_t shortaddr,uint8_t *pdata,uint8_t len)
{
    ...
    ZBM_SendBuffer[index++] = 0xC0;
    ZBM_SendBuffer[index++] = 0xC1;
    ZBM_SendBuffer[index++] = 0xC2;      
    for(i=0;i < len;i++)
    {
...
//==========================================================
//	函数名称：	ZBM_Send_Grounp
//
//	函数功能：	向指定的点发送数据
//
//	入口参数：	uint8_t grounpno,
//              uint8_t *pdata
//              uint8_t len
//              
//	返回参数：	返回类型ZBM_ErrorCode。
...
void ZBM_Send_Grounp(uint8_t grounpno,uint8_t *pdata,uint8_t len)
{
    ...
    ZBM_SendBuffer[index++] = 0xFC;
    ZBM_SendBuffer[index++] = len + 2+3;
    ZBM_SendBuffer[index++] = 0x02;
    ZBM_SendBuffer[index++] = grounpno;
    ZBM_SendBuffer[index++] = 0xC0;
    ZBM_SendBuffer[index++] = 0xC1;
    ZBM_SendBuffer[index++] = 0xC2;      
    for(i=0;i < len;i++)
...
void ZBM_Send_Broadcast(uint8_t broadcastmode,uint8_t *pdata,uint8_t len)
{
    ...
    ZBM_SendBuffer[index++] = 0xFC;
    ZBM_SendBuffer[index++] = len + 2+3;
    ZBM_SendBuffer[index++] = 0x01;
    ZBM_SendBuffer[index++] = broadcastmode;
    ZBM_SendBuffer[index++] = 0xC0;
    ZBM_SendBuffer[index++] = 0xC1;
    ZBM_SendBuffer[index++] = 0xC2;      
    for(i=0;i < len;i++)
