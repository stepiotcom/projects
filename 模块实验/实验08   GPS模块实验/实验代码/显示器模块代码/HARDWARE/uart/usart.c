...
uint8_t latitude[10] = 		"Latitude :";					//latitude
uint8_t longitude[10] = 		"Longitude:";				//longitude

uint8_t ffhh[2] = 			" -";										//负号
uint8_t ffhh2[2] = 			"  ";						


unsigned char  number1[10],number2[10];					//数组存放经纬度信息
unsigned char  number3[10],number4[10];					//数组存放经纬度信息	
  ...
  if(USART1_RX_STA&0x8000)
	{
		...
		strx1=strstr((const char*)USART1_RX_BUF,(const char*)"nsh->lcd");		//南北纬
		strx2=strstr((const char*)USART1_RX_BUF,(const char*)"ewh->lcd");		//东西经	
		strx3=strstr((const char*)USART1_RX_BUF,(const char*)"lat->lcd");		//纬度	
		strx4=strstr((const char*)USART1_RX_BUF,(const char*)"lon->lcd");		//经度	
		
		if(strx1)													//南北纬数据
		{	
				for(i = 8;i<18;i++)
				{
					number1[i-8] = USART1_RX_BUF[i];
				}
		}
...
void TFT_Update(void)
{
				...
				LCD_ShowString(210,50,number1); 			
				LCD_ShowString(210,100,number2); 	
				LCD_ShowString(115,50,number3);
				LCD_ShowString(115,100,number4);
