...
uint8_t weight[7] = "Weight:";		//重量
uint8_t Company[1] = "g";					//单位
  ...
  if(USART1_RX_STA&0x8000)
	{
		...
		strx=strstr((const char*)USART1_RX_BUF,(const char*)"->lcd");			//判断总线数据是否为传输给lcd的数据
		
		if(strx)											//如果数据为传输给lcd的数据则取出数据显示
		{	
			switch(len)
			{
				case 9: 
			...
			LCD_ShowNum(80,50,number,4);  //更新重量数据
